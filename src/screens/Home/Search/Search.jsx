...
const Search = () => {
  ...
  return (
    ...
    >
      <View style={tw`bg-primaryDark`}>
        <View style={tw`px-6 mt-10`}>
          <Text style={tw`text-white text-xl font-semibold mb-2`}>Search</Text>
          <View style={tw`flex-row items-center justify-between bg-white mb-4 px-4 rounded-xl`}>
            <MagnifyingGlassIcon color="gray" size={20} />
            <TextInput
              style={tw`flex-1 py-3 pl-4`}
      ...
      <View style={tw`bg-primaryDark pt-8`}>
        <ScrollView
      ...
      {!text ? (
        <>
          {!filters ? (
            <>
              <View style={tw`mt-8`}>
                <Text style={tw`px-6 font-bold text-xl`}>
                  Your search history
                </Text>
                <ScrollView
                  horizontal={true}
              ...
              <View style={tw`px-6`}>
                <Text style={tw`font-bold text-xl`}>Top categories</Text>
                <View style={tw`flex-row flex-wrap justify-between flex-1 w-full mt-2`}>
                  {categories.map((item, index) => (
                    <View
                      style={tw`w-[48%] rounded-lg overflow-hidden relative mb-4`}
                      key={index}
                    >
                      <Image style={tw`h-28 w-full`} source={item.image} />
                      <View
                        style={tw`absolute flex-row items-center justify-center left-0 w-full h-full flex-1 p-2`}
                      >
            ...
            <>
              {filters.length === 0 ? (
                <View style={tw`flex-1 justify-center items-center`}>
                  <Text style={{ fontSize: fontPixel(19), marginTop:48 }}>
